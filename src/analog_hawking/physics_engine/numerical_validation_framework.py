...
class ConvergenceAnalysis:
    ...
    @staticmethod
    def richardson_extrapolation_error(errors: List[float], orders: List[float],
...
class NumericalValidator:
    ...
    def validate_gradient_methods(self,
                                 grid_sizes: List[int] = None,
                                 test_cases: List[ValidationTestCase] = None) -> Dict[str, ValidationResults]:
        """
        ...
        for test_case in test_cases:
            ...
            result_2nd = self._test_gradient_method(
                test_case, grid_sizes, 'numpy_gradient',
                lambda x, y: np.gradient(y, x), '2nd_order'
            )
            ...
            if max(grid_sizes) >= 5:  # 4th-order needs at least 5 points
                result_4th = self._test_gradient_method(
                    test_case, grid_sizes, '4th_order_central',
                    lambda x, y: fd_solver.gradient_central_4th(x, y), '4th_order'
                )
    ...
    def _test_gradient_method(self,
                            test_case: ValidationTestCase,
                            grid_sizes: List[int],
                            method_name: str,
                            method_function: Callable,
                            order_label: str) -> ValidationResults:
        ...
        for n in grid_sizes:
            ...
            if test_case.analytical_derivative is not None:
                ...
                error = np.linalg.norm(computed_gradient - analytical_gradient) / np.linalg.norm(analytical_gradient)
                errors.append(error)

                # Compute analytical error for comparison
                analytical_error = np.linalg.norm(np.gradient(y, x) - analytical_gradient) / np.linalg.norm(analytical_gradient)
        ...
        if not passed_convergence:
            recommendations.append(f"Expected order {test_case.expected_convergence_order}, got {achieved_order:.2f}")
        ...
        return ValidationResults(
            ...
            analytical_comparison={'baseline_error': analytical_errors[-1] if analytical_errors else 0.0}
        )

    def validate_interpolation_methods(self,
                                     grid_sizes: List[int] = None,
                                     test_cases: List[ValidationTestCase] = None) -> Dict[str, ValidationResults]:
        ...
        for test_case in test_cases:
            ...
            result_linear = self._test_interpolation_method(
                test_case, grid_sizes, 'linear',
                lambda x, y, xi: np.interp(xi, x, y), 'linear'
            )
            test_results.append(result_linear)

            # Test cubic spline interpolation
            result_cubic = self._test_interpolation_method(
                test_case, grid_sizes, 'cubic_spline',
                lambda x, y, xi: interpolator.cubic_spline_interpolation(x, y)(xi), 'cubic'
            )
            test_results.append(result_cubic)

            # Test monotonic cubic interpolation
            result_monotonic = self._test_interpolation_method(
                test_case, grid_sizes, 'pchip',
                lambda x, y, xi: interpolator.cubic_spline_interpolation(x, y, monotonic=True)(xi), 'monotonic'
            )
    ...
    def _test_interpolation_method(self,
                                 test_case: ValidationTestCase,
                                 grid_sizes: List[int],
                                 method_name: str,
                                 method_function: Callable,
        ...
        for n in grid_sizes:
            ...
            # Compute error
            error = np.linalg.norm(y_test_interpolated - y_test_analytical) / np.linalg.norm(y_test_analytical)
            errors.append(error)
            execution_times.append(execution_time)
    ...
    def validate_horizon_detection(self,
                                 test_cases: List[ValidationTestCase] = None) -> Dict[str, ValidationResults]:
    ...
    def _test_horizon_detection_method(self,
                                     test_case: ValidationTestCase,
                                     grid_sizes: List[int],
                                     detector: EnhancedHorizonDetector,
                                     method_name: str) -> ValidationResults:
    ...
    def performance_benchmark(self, grid_size: int = 1000) -> Dict[str, PerformanceBenchmark]:
        ...
        accuracy_2nd = np.linalg.norm(grad_2nd - (10*np.pi*np.cos(10*np.pi*x) * np.exp(-x**2) -
        ...
        if grid_size >= 5:
            ...
            accuracy_4th = np.linalg.norm(grad_4th - (10*np.pi*np.cos(10*np.pi*x) * np.exp(-x**2) -
                                                      2*x*np.sin(10*np.pi*x) * np.exp(-x**2)))
        ...
        if 'gradient_methods' in self.validation_results:
            ...
            for test_name, test_results in self.validation_results['gradient_methods'].items():
                ...
                for result in test_results:
                    ...
                    report.append(f"    Convergence test: {'PASSED' if result.passed_convergence_test else 'FAILED'}")
