...
class EnhancedHorizonDetector:
    ...
    def find_horizons_enhanced(
        ...
        uncertainty_results = self._quantify_uncertainties(
            x, v, c_s, refined_horizons, kappa_results
        )
    ...
    def _refine_horizon_positions(
        self, x: np.ndarray, v: np.ndarray, c_s: np.ndarray, brackets: List[Tuple[float, float]]
    ) -> np.ndarray:
    ...
    def _compute_horizon_gradients(
        self, x: np.ndarray, v: np.ndarray, c_s: np.ndarray, horizon_positions: np.ndarray
    ) -> Dict[str, np.ndarray]:
        ...
        return {"dvdx": np.array(dvdx_list), "dcsdx": np.array(dcsdx_list)}
    ...
    def _compute_surface_gravity(
            ...
            convergence_info.append(
                {
                    ...
                    "sound_speed_at_horizon": (
                        c_h if "c_h" in locals() else self._interpolate_field(x, c_s, horizon_pos)
                    ),
        ...
        return {"kappa": np.array(kappa_list), "convergence_info": convergence_info}

    def _quantify_uncertainties(
        self,
        x: np.ndarray,
        v: np.ndarray,
        c_s: np.ndarray,
            ...
            for i, horizon_pos in enumerate(horizon_positions):
                ...
                for stencil in [1, 2, 3]:
                    if self.config.gradient_order == 4:
                        ...
                        dcsdx = (
                            c_s[min(idx + stencil, len(c_s) - 1)] - c_s[max(idx - stencil, 0)]
                        ) / (x[min(idx + stencil, len(x) - 1)] - x[max(idx - stencil, 0)])
        ...
        return {"kappa_err": np.array(kappa_err_list)}

    def _finite_difference_custom(
        self, x: np.ndarray, y: np.ndarray, idx: int, stencil: int
    ) -> float:
    ...
    def _perform_richardson_extrapolation(
        self, x: np.ndarray, v: np.ndarray, c_s: np.ndarray, horizon_positions: np.ndarray
    ...
    def _detect_physics_breakdown(
        ...
        physical_quantities = {"velocity": v, "sound_speed": c_s}
        ...
        diagnostic_functions = {"velocity": velocity_check, "sound_speed": sound_speed_check}
    ...
    def _create_interpolators(
        self, x: np.ndarray, v: np.ndarray, c_s: np.ndarray
    ) -> Dict[str, Callable]:
        ...
        if self.config.interpolation_method == "cubic_spline":
            ...
            interpolators["velocity"] = self.interpolator.cubic_spline_interpolation(
                x, v, monotonic=True
            )
            interpolators["sound_speed"] = self.interpolator.cubic_spline_interpolation(
                x, c_s, monotonic=True
            )
        else:  # linear
    ...
    def _generate_accuracy_report(
        ...
        if len(x) < 10:
            recommendations.append(
                "Grid resolution is low. Consider increasing grid points for better accuracy."
            )
...
def find_horizons_with_enhanced_accuracy(
    x: np.ndarray,
    v: np.ndarray,
    c_s: np.ndarray,
    ...
    **kwargs,
    ...
    config = HorizonDetectionConfig(
        gradient_order=gradient_order, kappa_method=kappa_method, **kwargs
    )
