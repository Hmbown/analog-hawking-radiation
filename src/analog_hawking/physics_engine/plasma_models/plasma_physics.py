...
class PlasmaPhysicsModel:
    ...
    def __init__(
        ...
        if self.a0 > 1:
            warnings.warn(
                f"Relativistic parameter a0 = {self.a0:.2f} > 1, relativistic effects important"
            )
...
class QEDPhysics:
    ...
    def pair_production_rate(self, E_field):
        ...
        if E_field < 0.1 * self.E_S:
            ...
            rate = (E_field**2 / (8 * np.pi**2 * self.lambda_C**3)) * np.exp(
                -np.pi * self.E_S / E_field
            )
        else:
...
def simulate_analog_black_hole(laser_params, plasma_params):
    ...
    density_profile = plasma.n_e * (
        1 + 0.1 * np.sin(2 * np.pi * z_grid / (2 * np.pi * c / plasma.omega_pe))
    )
    ...
    if isinstance(surface_gravity, (float, int, np.floating)):
        surface_gravity_value = float(surface_gravity)
    else:
        surface_gravity_array = np.asarray(surface_gravity)
        surface_gravity_value = (
            float(surface_gravity_array.flat[0]) if surface_gravity_array.size else 0.0
        )
...
if __name__ == "__main__":
    ...
    plasma_params = {"density": 1e18, "temperature": 10000}  # m^-3  # K
