...
class PhysicsBasedExperimentalDesign:
    ...
    def design_optimal_laser_parameters(self, target_temperature_range=(1e8, 1e10)):
        ...
        optimal_intensity = self._find_optimal_intensity_for_temperature(T_min, T_max)
    ...
    def _find_optimal_intensity_for_temperature(self, T_min, T_max, test_density=1e18):
        ...
        temperatures = [
            intensity_to_temperature(intensity_val) for intensity_val in test_intensities
        ]
...
def design_complete_experiment(experiment_type="anaBHEL"):
    ...
    optimization_result = design.optimize_for_detection_significance(
        laser_params, {"density": laser_params["plasma_density"]}, detector_params
    )
...
def assess_feasibility(laser_params, detector_params):
    ...
    energy_range_feasible = (
        detector_params["energy_range"][0] >= 10 and detector_params["energy_range"][1] <= 1e5
    )  # 10 eV to 100 keV
    ...
    return {
        ...
        "overall_feasible": all(
            [intensity_feasible, energy_feasible, repetition_feasible, energy_range_feasible]
        ),
...
if __name__ == "__main__":
    ...
    print(
        f"- Required measurement time: {experiment_design['measurement_requirements']['required_measurement_time']/3600:.1f} hours"
    )
