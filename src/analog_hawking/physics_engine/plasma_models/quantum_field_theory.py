...
class QuantumFieldTheory:
    ...
    def __init__(self, surface_gravity=1e12, temperature=None):
        """
    ...
    def hawking_spectrum(self, omega):
        """
        ...
        Returns:
            Power spectrum in arbitrary units
        """
        ...
        # Avoid numerical issues with very large exponentials
        with np.errstate(over='ignore'):
            spectrum = np.where(
                exp_factor > 1e10,  # For very large energies
                (h_omega**3 / (2 * np.pi**2 * c**2)) * np.exp(-h_omega / (k * self.T_H)),  # High energy tail
                (h_omega**3 / (2 * np.pi**2 * c**2)) / (exp_factor - 1)  # Standard Planck distribution
            )
        
        # For analog systems, we need to include the graybody factor
        # For simplicity, we use a transmission coefficient approximation
        graybody_factor = self.graybody_factor(omega)
        
        return spectrum * graybody_factor
    ...
    def graybody_factor(self, omega):
        ...
        # Use a simple form: T(ω) = 1 / (1 + exp(-4πω/κ)) for frequencies near the peak
        # This approximates the transmission through the effective potential
        return 1.0 / (1.0 + np.exp(-4 * np.pi * omega / self.kappa))
