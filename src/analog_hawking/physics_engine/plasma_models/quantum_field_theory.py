...
class QuantumFieldTheory:
    ...
    def __init__(
        self,
        surface_gravity=1e12,
        temperature=None,
        emitting_area_m2=None,
        solid_angle_sr=None,
    ...
    def hawking_spectrum(self, omega, transmission=None):
        ...
        if (self.emitting_area_m2 is not None) and (self.solid_angle_sr is not None):
            base_psd = (
                B_nu
                * float(self.emitting_area_m2)
                * float(self.solid_angle_sr)
                * float(self.coupling_efficiency)
            )
        else:
            base_psd = B_nu
    ...
    def thermal_spectral_density(self, frequency):
        ...
        with np.errstate(over="ignore", under="ignore", invalid="ignore"):
            ...
            spectral_density = np.where(x > 50.0, pre * np.exp(-x), pre / np.expm1(x))
    ...
    def energy_distribution(self, omega):
        ...
        with np.errstate(over="ignore", under="ignore", invalid="ignore"):
            occupation = np.where(x > 50.0, np.exp(-x), 1.0 / np.expm1(x))
    ...
    def occupation_number(self, omega):
        ...
        x = (hbar * omega) / (k * self.T_H)
        with np.errstate(over="ignore", under="ignore", invalid="ignore"):
            return np.where(x > 50.0, np.exp(-x), 1.0 / np.expm1(x))
...
class BogoliubovTransformations:
    ...
    def total_particle_flux(self, omega_range, surface_gravity):
        ...
        def integrand(omega):
            return (
                self.particle_creation_rate(omega, surface_gravity) * omega**2 / (np.pi**2 * c**3)
            )
...
class HawkingRadiationModel:
    ...
    def calculate_spectrum(self, frequency_range, num_points=1000):
        ...
        frequencies = np.logspace(
            np.log10(frequency_range[0]), np.log10(frequency_range[1]), num_points
        )
    ...
    def detectable_signal(self, detector_bandwidth, efficiency=0.5):
        ...
        total_power = np.trapz(
            spectrum_data["power_spectrum"] * efficiency, x=spectrum_data["frequencies"]
        )
        ...
        return {
            ...
            "snr": snr,
            "optimal_frequency": spectrum_data["frequencies"][
                np.argmax(spectrum_data["power_spectrum"])
            ],
