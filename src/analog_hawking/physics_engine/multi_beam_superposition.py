...
def _time_average_intensity(
    ...
    two_color: Optional[Dict[str, float]] = None,
) -> np.ndarray:
    """
...
def simulate_gradient_enhancement(
    config_name: str,
    ...
    I_single = _time_average_intensity(
        X, Y, beams_single, wavelength, w0, I_total, n_time, w0_lab=None, two_color=None
    )
    ...
    if config_name in ("ring", "angled_crossing"):
        beams_multi = _make_beams_from_config(
            config_name,
        ...
        beams_multi = _make_beams_from_config(
            config_name, weights=weights_list, phase_align=phase_align
        )
    I_multi = _time_average_intensity(
        X, Y, beams_multi, wavelength, w0, I_total, n_time, w0_lab=w0_lab, two_color=two_color
    )
...
def compare_configurations(configs: List[str] = None, **kwargs) -> Dict[str, Dict[str, float]]:
