...
class VariationPreservingArray:
    ...
    def collapse(self, operation: str = 'mean') -> float | np.ndarray:
        ...
        if operation == 'mean':
            ...
            # For std, we can't preserve variation in the same way, so compute directly
            result = np.std(self.data, axis=self.axis, keepdims=self.keepdims)
            ...
            tracker = VariationTracker(
                ...
                std_before=np.std(self.data),
                std_after=np.std(result) if hasattr(result, 'shape') else 0.0,
                metadata={'keepdims': self.keepdims}
            )
            self.variation_history.append(tracker)
...
def aggregate_patchwise_graybody(
    ...
    *,
    ...
    # Create common frequency grid that covers all spectra
    all_frequencies = []
    for sp in specs:
        all_frequencies.extend(sp["frequencies"])
    
    if not all_frequencies:
        return AggregatedSpectrum(success=False)
        
    # Use the global min/max frequency range with fixed number of points
    f_min = min(all_frequencies)
    f_max = max(all_frequencies)
    n_points = max(len(sp["frequencies"]) for sp in specs)
    f_common = np.linspace(f_min, f_max, n_points)
    ...
    for sp in specs:
        ...
        if preserve_variation:
            ...
            P_interp = P_variation.collapse('mean')
        else:
            P_interp = np.interp(f_common, f, P)
        P_mat.append(P_interp)
    
    # Stack and compute statistics with variation tracking
    P_stack = np.vstack(P_mat)
    f0 = f_common  # Use the common frequency grid
    ...
    if preserve_variation:
        ...
        P_std = P_std_variation.collapse('std')  # Now supports 'std' operation
    else:
