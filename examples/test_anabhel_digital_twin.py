...
def run_anabhel_digital_twin_experiment():
    ...
    plasma_params = {"density": exp_params["plasma_density"], "temperature": 10000}
    ...
    sim_results = simulation.simulate_laser_plasma_interaction(
        x_range=(-50e-6, 50e-6), t_range=(0, 100e-15), n_x=200, n_t=100
    )

    print(f"   Simulation completed with {len(sim_results['time_grid'])} time steps")
    print(
        f"   Spatial range: {sim_results['space_grid'][0]:.2e} to {sim_results['space_grid'][-1]:.2e} m"
    )
    print()
    ...
    if hawking_temp > 0:
        ...
        print(
            f"   Calculated spectrum over {len(hawking_spectrum['frequencies'])} frequency points"
        )
    ...
    if hawking_temp > 0:
        ...
        if time_for_snr_5 < float("inf"):
            print(
                f"   Time for 5σ detection: {time_for_snr_5:.2e} s ({time_for_snr_5/3600:.2f} hours)"
            )
        else:
    ...
    if hawking_temp > 0:
        ...
        noisy_data, freqs = bayes_analyzer.generate_synthetic_data(
            T_H_true=hawking_temp, kappa_true=max_surface_gravity, noise_level=0.1
        )
        ...
        print(
            f"   Inferred T_H: {param_results['T_H_mean']:.2e} ± {param_results['T_H_std']:.2e} K"
        )
        ...
        print(
            f"   Physical validation: {'✅ PASS' if validation['overall_consistent'] else '❌ FAIL'}"
        )
    else:
    ...
    print(
        f"   Overall model validation: {'✅ PASS' if validation_summary['overall_validity'] else '❌ FAIL'}"
    )
...
def visualize_results(results):
    ...
    if results["simulation_results"]:
        ...
        if "density_evolution" in results["simulation_results"]:
            ...
            axes[0, 0].text(
                0.5,
                0.5,
                "Density evolution data not available",
    ...
    if results["detectability"]:
        ...
        for t in times:
            ...
            snr = (
                signal_rate * t / np.sqrt((signal_rate + background_rate) * t)
...
def assess_detection_feasibility(results):
    ...
    if det_results:
        ...
        print(
            f"   Time for 5σ detection: {time_for_5sigma:.2e} s ({time_for_5sigma/3600:.2f} hours)"
        )
