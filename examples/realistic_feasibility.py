...
def run_realistic_anabhel_simulation():
    ...
    print(
        f"  Underdense condition (n < n_c): {'✅' if plasma_params['density'] < (constants.epsilon_0 * constants.m_e * omega_l**2 / constants.e**2) else '❌'}"
    )
    ...
    try:
        ...
        for t_idx in range(len(sim_results["time_grid"])):
            ...
            if len(horizon_positions) > 0:
                ...
                local_kappa = (
                    np.abs(dv_dx[horizon_positions]) / 2.0 if len(horizon_positions) > 0 else 0
                )
        ...
        if max_surface_gravity > 0:
            ...
            if hawking_temp > 1e6:  # 1 MK - reasonable for detection
                ...
                print(
                    f"   Peak detection frequency: {peak_freq:.2e} Hz (~{peak_freq*1e-15:.2f} PHz)"
                )

                # Estimate radiated power
                # For thermal spectrum: P = A * σ * T⁴ (Stefan-Boltzmann law)
                horizon_area = (
                    constants.hbar * constants.c / (constants.k * hawking_temp)
                ) ** 2  # Characteristic area
                ...
                if detected_power > detector_sensitivity:
                    ...
                    print(
                        f"   Signal-to-noise ratio would be ~{detected_power/detector_sensitivity:.0f}"
                    )
                    feasible = True
                else:
