...
def create_workflow_diagram():
    ...
    arrow_props = dict(
        arrowstyle="->,head_width=0.5,head_length=0.7", linewidth=2, color=arrow_color, zorder=1
    )
    ...
    for i in range(len(main_boxes) - 1):
        ...
        arrow = FancyArrowPatch((x_start, y_main), (x_end, y_main), **arrow_props)
        ax.add_patch(arrow)
    ...
    ax.text(
        6.0, 5.1, subtitle, ha="center", va="center", fontsize=11, style="italic", color=arrow_color
    )

    # Add key insight boxes
    insight_props = dict(
        boxstyle="round,pad=0.3", facecolor="#FFF9E6", edgecolor=edge_color, linewidth=1
    )
    ...
    ax.text(
        ...
        style="italic",
        fontweight="bold",
        bbox=dict(boxstyle="round,pad=0.4", facecolor="#FFE6E6", edgecolor=spec_color, linewidth=2),
        color=spec_color,
    )
