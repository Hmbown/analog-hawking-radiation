...
def analyze_cs_profile_impact():
    ...
    analog_model_const = AnalogHorizonFormation(maxwell_model, plasma_temperature_profile=const_T)
    ...
    analog_model_profile = AnalogHorizonFormation(
        maxwell_model, plasma_temperature_profile=T_profile
    )
    ...
    ax1.plot(
        x_grid * 1e6, np.full_like(x_grid, c_s_const) / c, "b--", label=r"$c_s/c$ (Constant T)"
    )
    ...
    if horizon_const_indices.any():
        ax1.plot(
            x_grid[horizon_const_indices] * 1e6,
            v_fluid[horizon_const_indices] / c,
            "bo",
            markersize=10,
            label="Horizon (Constant T)",
        )
    if horizon_profile_indices.any():
        ax1.plot(
            x_grid[horizon_profile_indices] * 1e6,
            v_fluid[horizon_profile_indices] / c,
            "ro",
            markersize=10,
            label="Horizon (Profiled T)",
        )
