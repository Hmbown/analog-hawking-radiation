...
def design_multi_mirror_configurations():
    ...
    for name, config in configurations.items():
        ...
        print(
            f"    Illustrative enhancement (conceptual): {config['expected_gradient_enhancement']}x"
        )
...
def design_optimized_laser_parameters():
    ...
    for name, params in laser_optimizations.items():
        ...
        print(
            f"    Intensity range: {params['intensity_range'][0]:.1e} - {params['intensity_range'][1]:.1e} W/m²"
        )
        if "wavelength" in params:
            wavelength_val = (
                params["wavelength"]
                if not isinstance(params["wavelength"], list)
                else params["wavelength"][0]
            )
            print(f"    Wavelength: {wavelength_val:.1e} m")
        elif "wavelength_range" in params:
            print(
                f"    Wavelength: {params['wavelength_range'][0]:.1e} - {params['wavelength_range'][1]:.1e} m"
            )
        print(
            f"    Pulse duration: {params['pulse_duration_range'][0]*1e15:.0f} - {params['pulse_duration_range'][1]*1e15:.0f} fs"
        )
        print(
            f"    Focus diameter: {params['focus_diameter_range'][0]*1e6:.1f} - {params['focus_diameter_range'][1]*1e6:.1f} μm"
        )
...
def calculate_multi_mirror_enhancement_factors(
    plasma_density: float = 1e18, electron_temperature: float = 10000, tau_response: float = None
):
    """
...
def create_multi_mirror_visualization():
    ...
    print(
        "  ✅ Multi-mirror configuration visualization saved as 'multi_mirror_configurations.png'"
    )
...
def main_multi_mirror_analysis():
    ...
    print(
        "2. Pentagram/standing-wave appear promising; factors now come from field superposition, not multipliers"
    )
