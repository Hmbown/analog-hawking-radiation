...
def test_relativistic_a0():
    ...
    plasma = PlasmaPhysicsModel(
        plasma_density=1e18, laser_wavelength=wavelength, laser_intensity=intensity
    )
...
def test_acoustic_metric():
    ...
    analog = AnalogHorizonFormation(
        plasma, plasma_temperature_profile=c_sound**2 * 1.16e4 * 1.6e-19 / (5 / 3)
    )  # T_e in eV
    ...
    print(
        f"  g_tt - Expected: {expected_g_tt:.2e}, Calculated: {metric['g_tt']:.2e}, Error: {error_g_tt:.2e}"
    )
    print(
        f"  g_tx - Expected: {expected_g_tx:.2e}, Calculated: {metric['g_tx']:.2e}, Error: {error_g_tx:.2e}"
    )
...
def test_wakefield_amplitude():
    ...
    laser_params = {"intensity": 1e18, "wavelength": 800e-9, "duration": 30e-15}

    plasma_params = {"density": 1e18, "temperature": 10000}
    ...
    try:
        sim_result = simulation.simulate_laser_plasma_interaction(
            x_range=(-20e-6, 20e-6), t_range=(0, 50e-15), n_x=50, n_t=25
        )
