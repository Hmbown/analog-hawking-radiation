...
def plot_guidance_map():
    ...
    optimal_params = {"plasma_density": 4.758e24, "laser_intensity": 1.152e22, "T_peak": 1.115e07}
    ...
    for i, intensity in enumerate(laser_intensity_range):
        for j, density in enumerate(plasma_density_range):
            print(
                f"  Calculating merit for point ({i+1}, {j+1}) of ({len(laser_intensity_range)}, {len(plasma_density_range)})..."
            )
    ...
    im = ax.imshow(
        ...
        aspect="auto",
        extent=[
            plasma_density_range[0],
            plasma_density_range[-1],
            laser_intensity_range[0],
            laser_intensity_range[-1],
        ],
    ...
    ax.plot(
        optimal_params["plasma_density"],
        optimal_params["laser_intensity"],
        "r*",
        markersize=15,
        label="Optimal Point",
    )
