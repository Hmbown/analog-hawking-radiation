...
class ExperimentTracker:
    ...
    def __init__(
        self, experiment_name: Optional[str] = None, base_dir: str = "results/orchestration"
    ):
    ...
    def _capture_provenance(self) -> None:
        ...
        self.manifest.provenance.hostname = (
    ...
    def complete_experiment(self) -> None:
        ...
        if self.manifest.metrics.total_simulations > 0:
            self.manifest.metrics.average_simulation_time = (
                self.manifest.metrics.total_compute_time / self.manifest.metrics.total_simulations
            )
            ...
            if phase_record["phase_name"] == phase_name:
                ...
                phase_record["results_summary"] = {
                    ...
                    "successful_simulations": sum(
                        1 for r in phase_results.get("results", []) if r.get("simulation_success")
                    ),
    ...
    def get_experiment_summary(self) -> Dict[str, Any]:
        ...
        if self.manifest.metrics.total_simulations > 0:
            success_rate = (
                self.manifest.metrics.successful_simulations
        ...
        return {
            ...
            "phases_completed": len(
                [p for p in self.manifest.phase_history if p.get("status") == "completed"]
            ),
...
class ExperimentRegistry:
    ...
    def search_experiments(self, query: str) -> List[Dict[str, Any]]:
        ...
        for experiment in self.registry.values():
            if (
                query in experiment.get("name", "").lower()
                or query in experiment.get("experiment_id", "").lower()
            ):
