...
class SurrogateTrainer:
    ...
    def train_gaussian_process(self, X: np.ndarray, y: np.ndarray) -> Dict[str, Any]:
        ...
        if self.config.kernel_type == "rbf":
            kernel = C(1.0, (1e-3, 1e3)) * RBF([1.0] * X.shape[1], (1e-2, 1e2)) + WhiteKernel(
                noise_level=1.0
            )
        else:  # matern
            kernel = C(1.0, (1e-3, 1e3)) * Matern(
                [1.0] * X.shape[1], (1e-2, 1e2), nu=1.5
            ) + WhiteKernel(noise_level=1.0)

        # Train model
        gp = GaussianProcessRegressor(
            kernel=kernel, n_restarts_optimizer=10, random_state=self.config.random_state
        )
    ...
    def train_neural_network(self, X: np.ndarray, y: np.ndarray) -> Dict[str, Any]:
        ...
        model = HawkingNeuralNetwork(
            input_dim=X.shape[1], hidden_layers=self.config.hidden_layers
        ).to(device)
    ...
    def train_ensemble(self, X: np.ndarray, y: np.ndarray) -> Dict[str, Any]:
        ...
        metrics["ensemble_metrics"] = {"model_comparison": metrics, "selected_model": best_model}
    ...
    def predict(self, X: np.ndarray, model_type: str = None) -> np.ndarray:
        ...
        if model_type == "nn":
            ...
            # Inverse transform
            predictions = (
                self.scalers["nn_target"].inverse_transform(predictions.reshape(-1, 1)).ravel()
            )
...
def main():
    parser = argparse.ArgumentParser(
        description="Train surrogate models for Hawking radiation detection"
    )
    parser.add_argument("--data", required=True, help="Path to training data (JSON files)")
    parser.add_argument(
        "--model",
        default="ensemble",
        choices=["gp", "rf", "nn", "ensemble"],
