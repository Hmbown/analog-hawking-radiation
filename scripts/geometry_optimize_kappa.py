...
def main() -> int:
    ...
    if args.three_d:
        ...
        for conf in configs_3d:
            if conf == "single_mirror":
                kappa_3d = (
                    np.max(np.gradient(density_3d)[0]) * 3e5 / np.mean(density_3d)
                )  # Approximate kappa
                ...
                kappa_3d = (
                    10 * np.max(np.gradient(density_3d)[0]) * 3e5 / np.mean(density_3d)
                )  # Target 10x
            boosts.append(float(kappa_3d))
            T_H = (
                1.38e-23 * kappa_3d / (2 * np.pi) * 1e9 / 1e-3
            )  # Approximate T_H in mK GHz units? Wait, adjust
    ...
    with open("results/geometry_vs_kappa.json", "w") as f:
        json.dump(
            {"order": order, "enhancement": enh, "kappa_surrogate_enhancement": kappa_sur},
            f,
            indent=2,
        )
