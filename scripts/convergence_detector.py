...
class AdvancedConvergenceDetector:
    ...
    def add_results(self, results: List[Dict[str, Any]]) -> None:
        ...
        if current_detection_times:
            param_diversity = (
                np.std(np.log10(current_detection_times))
                if len(current_detection_times) > 1
                else 0.0
            )
        ...
        for history in [
            ...
            self.parameter_diversity_history,
        ]:
    ...
    def check_convergence(self) -> ConvergenceResult:
        ...
        enough_data_for_convergence = len(self.detection_time_history) >= (
    ...
    def _generate_recommendations(
        self, tests: List[Tuple[bool, float, str]], convergence_score: float
    ) -> List[str]:
        ...
        for test in tests:
            ...
            if "moving_avg" in test_id and not test_passed:
                recommendations.append(
                    "Improvement rate is still significant - continue current phase"
                )

            if "statistical_test" in test_id and not test_passed:
                recommendations.append(
                    "Statistical significance not yet achieved - collect more data"
                )
...
class MultiPhaseConvergenceManager:
    ...
    def check_phase_transition(self, current_phase: str, next_phase: str) -> Tuple[bool, List[str]]:
        ...
        if should_transition:
            ...
            self.logger.info(
                f"Convergence score: {convergence_result.metrics.overall_convergence_score:.3f}"
            )
        else:
...
def main():
    ...
    for i in range(50):
        ...
        if i >= 10:  # Start checking after some data
            ...
            print(
                f"Iteration {i:2d}: Score={result.metrics.overall_convergence_score:.3f}, "
