...
class BenchmarkValidator:
    ...
    def run_comprehensive_benchmark_validation(self) -> BenchmarkValidationSummary:
        ...
        self.logger.info(
            f"Starting comprehensive benchmark validation for experiment {self.experiment_id}"
        )
        ...
        self.logger.info(
            f"Benchmark validation completed: {summary.passed_benchmarks}/{summary.total_benchmarks} benchmarks passed"
        )
    ...
    def validate_kappa_against_analytical(self) -> List[BenchmarkResult]:
        ...
        if not all_kappas:
            return [
                BenchmarkResult(
                    ...
                    confidence=0.0,
                    metric_value=0.0,
                    reference_value=self.analytical_benchmarks["standard_hawking_temperature"][
                        "kappa_reference"
                    ],
                    tolerance=self.tolerances["kappa_analytical"],
                    relative_error=1.0,
        ...
        reference_kappa = self.analytical_benchmarks["standard_hawking_temperature"][
            "kappa_reference"
        ]
    ...
    def validate_detection_time_scaling(self) -> List[BenchmarkResult]:
        ...
        if not self.results:
            return [
                BenchmarkResult(
                    ...
                    confidence=0.0,
                    metric_value=0.0,
                    reference_value=self.analytical_benchmarks["detection_time_scaling"][
                        "expected_exponent"
                    ],
        ...
        try:
            ...
            expected_exponent = self.analytical_benchmarks["detection_time_scaling"][
                "expected_exponent"
            ]
    ...
    def validate_temperature_scaling(self) -> List[BenchmarkResult]:
        ...
        for phase_results in self.results.values():
            for result in phase_results:
                if result.get("simulation_success") and result.get("T_sig_K") is not None:
        ...
        results.append(
            BenchmarkResult(
                ...
                details={
                    ...
                    "detection_probability_3sigma_1d": detection_stats_1d.get(
                        "detection_probability_3sigma", 0.0
                    ),
                    "detection_probability_5sigma_1h": detection_stats_1h.get(
            ...
            if not result.passed:
                if "kappa" in result.benchmark_name.lower():
                    ...
                    recommendations.append(
                        "Investigate scaling law deviations - check model assumptions"
                    )
    ...
    def save_benchmark_report(
        self, summary: BenchmarkValidationSummary, output_path: Optional[Path] = None
    ) -> None:
    ...
    def generate_benchmark_report_text(self, summary: BenchmarkValidationSummary) -> str:
        ...
        for result in summary.benchmark_results:
            ...
            report += (
                f"  Metric: {result.metric_value:.3e}, Reference: {result.reference_value:.3e}\n"
            )
