...
class CrossPhaseValidator:
    ...
    def run_comprehensive_cross_phase_validation(self) -> CrossPhaseValidationSummary:
        ...
        self.logger.info(
            f"Starting comprehensive cross-phase validation for experiment {self.experiment_id}"
        )
        ...
        self.logger.info(
            f"Cross-phase validation completed: {summary.valid_transitions}/{summary.total_transitions} valid transitions"
        )
    ...
    def validate_phase_transition(self, from_phase: str, to_phase: str) -> PhaseTransitionResult:
        ...
        transition_score = self._calculate_transition_score(
            improvement_metrics, consistency_metrics
        )
        ...
        message = self._generate_transition_message(
            from_phase, to_phase, improvement_metrics, consistency_metrics
        )
        recommendations = self._generate_transition_recommendations(
            improvement_metrics, consistency_metrics, passed
        )
    ...
    def _calculate_improvement_metrics(self, from_phase: str, to_phase: str) -> Dict[str, float]:
        ...
        if from_kpis["best_detection_time"] and to_kpis["best_detection_time"]:
            improvement = (
                from_kpis["best_detection_time"] - to_kpis["best_detection_time"]
            ) / from_kpis["best_detection_time"]
            metrics["detection_time_improvement"] = max(
                0.0, improvement
            )  # Negative improvement = degradation

        # Kappa improvement (higher is better)
        if from_kpis["best_kappa"] and to_kpis["best_kappa"]:
            improvement = (to_kpis["best_kappa"] - from_kpis["best_kappa"]) / from_kpis[
                "best_kappa"
            ]
        ...
        if from_kpis["detection_probability_5sigma"] and to_kpis["detection_probability_5sigma"]:
            improvement = (
                to_kpis["detection_probability_5sigma"] - from_kpis["detection_probability_5sigma"]
            )
    ...
    def _calculate_distribution_similarity(
        self, results1: List[Dict], results2: List[Dict], metric: str, aggregation: str = "direct"
    ) -> Optional[float]:
    ...
    def _extract_metric_values(
        self, results: List[Dict], metric: str, aggregation: str = "direct"
    ) -> List[float]:
    ...
    def _calculate_statistical_consistency(
    ...
    def _extract_phase_kpis(self, phase_name: str) -> Dict[str, Any]:
        ...
        detection_times = [
            r.get("t5sigma_s") for r in successful_results if r.get("t5sigma_s") is not None
        ]
    ...
    def _calculate_transition_score(
        self, improvement_metrics: Dict[str, float], consistency_metrics: Dict[str, float]
    ) -> float:
        """Calculate overall transition score"""
        ...
        # Combined score
        transition_score = (
            improvement_weight * avg_improvement + consistency_weight * avg_consistency
        )
    ...
    def analyze_progression_patterns(self, phase_order: List[str]) -> Dict[str, Any]:
        ...
        progression = {
            "detection_time_trend": self._analyze_metric_trend(
                phase_order, "t5sigma_s", "decreasing"
            ),
            "kappa_trend": self._analyze_metric_trend(
                phase_order, "kappa", "increasing", aggregation="max"
            ),
            "success_rate_trend": self._analyze_metric_trend(
                phase_order, "success_rate", "increasing"
            ),
    ...
    def _analyze_convergence_progression(self, phase_order: List[str]) -> Dict[str, Any]:
        ...
        for phase in phase_order:
            ...
            if len(detection_times) > 5:
                ...
                cv = (
                    np.std(detection_times) / np.mean(detection_times)
        ...
        improving = all(
            convergence_scores[i] <= convergence_scores[i + 1]
        ...
        return {
            "progression": (
                "improving"
                if improving
                else "stable" if len(set(convergence_scores)) == 1 else "fluctuating"
            ),
            "improving": improving,
    ...
    def _generate_summary(
        self, transition_results: List[PhaseTransitionResult], progression_analysis: Dict[str, Any]
    ) -> CrossPhaseValidationSummary:
        ...
        recommendations = self._generate_overall_recommendations(
            transition_results, progression_analysis
        )
    ...
    def _generate_overall_recommendations(
        ...
        success_rate = (
            len([r for r in transition_results if r.passed]) / len(transition_results)
            if transition_results
            else 0.0
        )
        ...
        if kappa_trend.get("trend") == "decreasing":
            recommendations.append(
                "Surface gravity values are decreasing - focus on horizon optimization"
            )
        ...
        if not convergence_progression.get("improving", False):
            recommendations.append(
                "Convergence is not improving across phases - review optimization strategy"
            )

        return recommendations

    def save_cross_phase_report(
        self, summary: CrossPhaseValidationSummary, output_path: Optional[Path] = None
    ) -> None:
