...
class QualityAssuranceSystem:
    ...
    def run_comprehensive_quality_assurance(self) -> QualityAssuranceReport:
        ...
        self.logger.info(
            f"Starting comprehensive quality assurance for experiment {self.experiment_id}"
        )
        ...
        recommendations = self._generate_recommendations(
            quality_metrics, anomalies, overall_quality_score
        )
        ...
        self.logger.info(
            f"Quality assurance completed: overall score = {overall_quality_score:.3f}"
        )
    ...
    def check_data_completeness(self) -> QualityMetric:
        ...
        for phase_name, phase_results in self.results.items():
            for result in phase_results:
                ...
                has_valid_values = (
                    result.get("simulation_success") is not None
                    and result.get("t5sigma_s") is not None
                    and result.get("kappa") is not None
                    and len(result.get("kappa", [])) > 0
                    and result.get("T_sig_K") is not None
                )
        ...
        completeness_score = (
            complete_simulations / total_simulations if total_simulations > 0 else 0.0
        )
    ...
    def check_parameter_boundaries(self) -> QualityMetric:
        ...
        score = max(
            0.0, 1.0 - violation_rate / self.thresholds["parameter_boundary_violations_max"]
        )
    ...
    def check_statistical_consistency(self) -> QualityMetric:
        ...
        cv = (
            np.std(detection_times_log) / np.mean(detection_times_log)
            if np.mean(detection_times_log) != 0
            else 1.0
        )
    ...
    def check_physical_plausibility(self) -> QualityMetric:
        ...
        temperature_plausibility = (
            len(plausible_temperatures) / len(all_temperatures) if all_temperatures else 1.0
        )

        # Check detection times (reasonable range for observatories)
        plausible_detection_times = [
            t for t in all_detection_times if 1 <= t <= 1e7
        ]  # 1s to ~115 days
        detection_time_plausibility = (
            len(plausible_detection_times) / len(all_detection_times)
        ...
        plausibility_score = (
            kappa_plausibility + temperature_plausibility + detection_time_plausibility
        ) / 3.0
    ...
    def _generate_recommendations(
        ...
        if anomalies.anomalies_detected > 0:
            ...
            if anomaly_rate > 0.1:
                recommendations.append(
                    f"Investigate {anomalies.anomalies_detected} anomalous results detected"
                )

        # General recommendations based on overall score
        if overall_score < 0.5:
            recommendations.append(
                "Significant quality issues detected - comprehensive review recommended"
            )
    ...
    def save_quality_report(
        self, report: QualityAssuranceReport, output_path: Optional[Path] = None
    ) -> None:
    ...
    def generate_quality_report_text(self, report: QualityAssuranceReport) -> str:
        ...
        if report.parameter_boundary_violations:
            ...
            if len(report.parameter_boundary_violations) > 10:
                quality_text += (
                    f"... and {len(report.parameter_boundary_violations) - 10} more violations\n"
                )
        else:
