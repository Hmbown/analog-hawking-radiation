...
class ReportGenerator:
    ...
    def _generate_discussion(self) -> str:
        ...
        if sensitivity:
            ...
            discussion += (
                f"{most_sensitive} exhibited the strongest influence on detection efficiency, "
            )
        else:
            discussion += (
                "laser intensity exhibited the strongest influence on detection efficiency, "
            )
    ...
    def _calculate_success_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "validation_confidence": (
                self.validation_summary.overall_confidence if self.validation_summary else 0.0
            ),
    ...
    def _assess_risks(self) -> Dict[str, Any]:
        ...
        risks = {"technical_risks": [], "implementation_risks": [], "validation_risks": []}

        if self.aggregate:
            if self.aggregate.success_rate < 0.5:
                risks["technical_risks"].append(
                    "Moderate success rate may limit experimental applicability"
                )

            if not self.aggregate.cross_phase_correlation.significant_correlations:
                risks["validation_risks"].append(
                    "Limited cross-phase consistency requires additional validation"
                )
    ...
    def _calculate_improvement(self) -> float:
        ...
        if first_phase.get("best_detection_time") and last_phase.get("best_detection_time"):
            improvement = (
                first_phase["best_detection_time"] - last_phase["best_detection_time"]
            ) / first_phase["best_detection_time"]
    ...
    def _extract_technical_insights(self) -> List[str]:
        ...
        if self.aggregate and self.aggregate.parameter_sensitivity:
            sensitive_params = [
                p for p, s in self.aggregate.parameter_sensitivity.items() if s > 0.3
            ]
            if sensitive_params:
                insights.append(
                    f"High sensitivity to {', '.join(sensitive_params)} suggests focused experimental control"
                )
    ...
    def _fit_distribution(self, data: List[float]) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "goodness_of_fit": stats.kstest(
                    data, "lognorm", args=(sigma, 0, np.exp(mu))
                ).pvalue,
            }
        except:
...
def main():
    ...
    parser.add_argument(
        "--report_type",
        choices=["all", "scientific", "executive", "technical"],
        default="all",
