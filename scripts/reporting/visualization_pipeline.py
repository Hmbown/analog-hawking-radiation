...
class VisualizationPipeline:
    ...
    def _generate_phase_progression_figure(self, spec: FigureSpecification) -> List[Path]:
        ...
        best_times = [self.aggregate.phase_summary[p].get("best_detection_time") or np.nan for p in phases]
        ...
        if valid_indices:
            ...
            ax1.plot(valid_indices, valid_times, 'o-', linewidth=2, markersize=8, 
                    color='blue', label='Best Detection Time')
            ax1.set_yscale('log')
        ...
        if valid_kappa_indices:
            ...
            ax2.plot(valid_kappa_indices, valid_kappas, 's-', linewidth=2, markersize=8,
                    color='orange', label='Best Surface Gravity')
            ax2.set_yscale('log')
        ...
        if self.aggregate.parameter_sensitivity:
            ...
            # Add threshold line
            ax4.axvline(x=0.3, color='gray', linestyle='--', alpha=0.7, label='Significance Threshold')
            ax4.legend()
    ...
    def _generate_bar_chart(self, spec: FigureSpecification) -> List[Path]:
        ...
        bars = ax.barh(param_names, sensitivities, 
    ...
    def _generate_heatmap(self, spec: FigureSpecification) -> List[Path]:
        ...
        if not self.aggregate or not self.aggregate.cross_phase_correlation.correlation_coefficients:
            return []
        ...
        for (phase1, phase2), corr in self.aggregate.cross_phase_correlation.correlation_coefficients.items():
    ...
    def _generate_distribution_plot(self, spec: FigureSpecification) -> List[Path]:
        ...
        n, bins, patches = ax1.hist(np.log10(detection_times), bins=30, alpha=0.7, 
        ...
        if phase_detection_times:
            phase_labels = [p.replace('phase_', '').replace('_', ' ').title() 
    ...
    def _generate_line_plot(self, spec: FigureSpecification) -> List[Path]:
        ...
        if not self.aggregate or 'detection_probability_1h' not in self.aggregate.statistical_significance:
            return []
        ...
        for sigma in sigma_levels:
            probs = []
            for time_key in ['detection_probability_1h', 'detection_probability_1d', 'detection_probability_1w']:
        ...
        for i, (sigma, probs) in enumerate(probabilities.items()):
            if len(probs) == len(observation_times):
                ax.plot(observation_times, probs, 'o-', linewidth=2, markersize=8,
                       color=colors[i], label=f'{sigma}σ Detection')
    ...
    def _generate_radar_chart(self, spec: FigureSpecification) -> List[Path]:
        ...
        fig, ax = plt.subplots(figsize=(spec.width, spec.height), subplot_kw=dict(projection='polar'))
    ...
    def _generate_scatter_plot(self, spec: FigureSpecification) -> List[Path]:
        ...
        if detection_times and kappa_values:
            ...
            scatter = ax.scatter(np.log10(detection_times), np.log10(kappa_values),
                               c=np.arange(len(detection_times)), cmap='viridis',
                               alpha=0.6, s=50)
