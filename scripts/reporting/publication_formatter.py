            ...
            tables.append(
                {
                    ...
                    "content": self._create_latex_parameter_sensitivity_table(
                        results_data["parameter_sensitivity"]
                    ),
                    "placement": "htbp",
                }
            )
            ...
            tables.append(
                {
                    ...
                    "content": self._create_latex_phase_summary_table(
                        results_data["phase_progression"]
                    ),
                    "placement": "htbp",
                }
            )
...
"""
        ...
        table += (
            f"Best Detection Time & \\SI{{{optimal['best_detection_time']:.2e}}}{{\\second}} \\\\\n"
        )
    ...
    def _write_latex_document(self, latex_doc: LaTeXDocument) -> None:
        ...
        with open(latex_file, "w") as f:
            ...
            for figure in latex_doc.figures:
                ...
                f.write("\\centering\n")
                f.write(
                    "\\includegraphics[width=" + figure["width"] + "]{" + figure["file"] + "}\n"
                )
...
""",
    ...
    def _generate_markdown_tables(self) -> List[Dict[str, Any]]:
        ...
        if results_data.get("phase_progression"):
            ...
            tables.append({"title": "Phase Performance Summary", "content": table_content})
    ...
    def _generate_slide_content(self) -> List[Dict[str, Any]]:
        ...
        if results_data.get("parameter_sensitivity"):
            ...
            for param, sensitivity in list(results_data["parameter_sensitivity"].items())[
                :3
            ]:  # Top 3
                param_display = param.replace("_", " ").title()
...
<head>
    ...
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/reveal.js/4.3.1/theme/"""
                + slides.theme
                + """.min.css">
    <style>
...
"""
    ...
    def _create_optimization_trajectory_table(self) -> pd.DataFrame:
        ...
        data = {"Phase": [], "Best Detection Time (s)": [], "Success Rate": [], "Simulations": []}
    ...
    def _export_data_tables(self, data_tables: DataTables) -> None:
        ...
        # Export to CSV
        data_tables.experiment_summary.to_csv(
            self.publication_dir / "experiment_summary.csv", index=False
        )
        data_tables.parameter_sensitivity.to_csv(
            self.publication_dir / "parameter_sensitivity.csv", index=False
        )
        data_tables.statistical_results.to_csv(
            self.publication_dir / "statistical_results.csv", index=False
        )
        data_tables.validation_metrics.to_csv(
            self.publication_dir / "validation_metrics.csv", index=False
        )
        data_tables.optimization_trajectory.to_csv(
            self.publication_dir / "optimization_trajectory.csv", index=False
        )

        # Export to Excel with multiple sheets
        with pd.ExcelWriter(self.publication_dir / "all_data_tables.xlsx") as writer:
            data_tables.experiment_summary.to_excel(
                writer, sheet_name="Experiment Summary", index=False
            )
            data_tables.parameter_sensitivity.to_excel(
                writer, sheet_name="Parameter Sensitivity", index=False
            )
            data_tables.statistical_results.to_excel(
                writer, sheet_name="Statistical Results", index=False
            )
            data_tables.validation_metrics.to_excel(
                writer, sheet_name="Validation Metrics", index=False
            )
            data_tables.optimization_trajectory.to_excel(
                writer, sheet_name="Optimization Trajectory", index=False
            )
...
def main():
    ...
    parser.add_argument(
        "--format",
        choices=["all", "latex", "markdown", "slides", "tables"],
        default="all",
