...
class ParameterSweep:
    ...
    def __init__(self, config: SweepConfig):
        ...
        logging.basicConfig(
            ...
            handlers=[logging.FileHandler(self.output_dir / "sweep.log"), logging.StreamHandler()],
        )
    ...
    def generate_parameter_grid(self) -> List[Dict[str, float]]:
        ...
        intensity_vals = np.logspace(
            ...
            int(
                (self.config.laser_intensity_range[1] / self.config.laser_intensity_range[0]) ** 0.5
                * 10
            ),
        )
        ...
        density_vals = np.logspace(
            ...
            int(
                (self.config.plasma_density_range[1] / self.config.plasma_density_range[0]) ** 0.5
                * 10
            ),
        )
        ...
        magnetic_vals = np.linspace(
            ...
            max(
                5,
                int(
                    (self.config.magnetic_field_range[1] - self.config.magnetic_field_range[0]) / 20
                )
                + 1,
            ),
        )
        ...
        grid_size_vals = np.linspace(
            self.config.grid_size_range[0], self.config.grid_size_range[1], 8
        )

        hybrid_D_vals = np.logspace(
            np.log10(self.config.hybrid_D_range[0]), np.log10(self.config.hybrid_D_range[1]), 6
        )

        hybrid_eta_vals = np.linspace(
            self.config.hybrid_eta_range[0], self.config.hybrid_eta_range[1], 5
        )
