...
class RealTimeDashboard:
    ...
    def __init__(self, experiment_id: str, update_interval: float = 5.0):
        if not RICH_AVAILABLE:
            raise RuntimeError("rich is not installed; RealTimeDashboard is unavailable. Use ExperimentDashboard instead.")
        self.experiment_id = experiment_id
        self.update_interval = update_interval
    ...
    def _create_progress_panel(self, metrics: DashboardMetrics) -> Panel:
        ...
        # Progress bar
        progress_bar = "█" * int(metrics.phase_progress * 20) + "░" * (20 - int(metrics.phase_progress * 20))
    ...
    def _create_metrics_panel(self, metrics: DashboardMetrics) -> Panel:
        ...
        table.add_row("Total Compute Time", f"{metrics.timestamp - (self.experiment_data.get('start_time', metrics.timestamp) if self.experiment_data else metrics.timestamp):.0f}s")
    ...
    def _create_system_panel(self, metrics: DashboardMetrics) -> Panel:
        ...
        # CPU
        cpu_bar = "█" * int(metrics.system_cpu_percent / 5) + "░" * (20 - int(metrics.system_cpu_percent / 5))
        table.add_row("CPU", f"{metrics.system_cpu_percent:.1f}%", cpu_bar)
        
        # Memory
        memory_bar = "█" * int(metrics.system_memory_percent / 5) + "░" * (20 - int(metrics.system_memory_percent / 5))
        table.add_row("Memory", f"{metrics.system_memory_percent:.1f}%", memory_bar)
        
        # Disk
        disk_bar = "█" * int(metrics.system_disk_percent / 5) + "░" * (20 - int(metrics.system_disk_percent / 5))
    ...
    def _create_convergence_panel(self, metrics: DashboardMetrics) -> Panel:
        ...
        convergence_bar = "█" * int(metrics.convergence_score * 10) + "░" * (10 - int(metrics.convergence_score * 10))
...
"""
    ...
    def _create_performance_panel(self, metrics: DashboardMetrics) -> Panel:
        ...
        if not metrics.performance_issues:
            return Panel("No performance issues detected", title="Performance", title_align="left", style="green")
    ...
    def export_metrics(self, output_path: Optional[Path] = None) -> None:
        ...
        if not output_path:
            output_path = Path("results/orchestration") / self.experiment_id / "dashboard_metrics.json"
