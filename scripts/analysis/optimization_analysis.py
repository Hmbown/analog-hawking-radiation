...
class HawkingRadiationOptimizer:
    ...
    def identify_pareto_frontier(
        self,
        objectives=["T_sig_hybrid", "t5_hybrid"],
    ...
    def multi_objective_optimization(self, objective_weights=None):
        ...
        for obj, weight in objective_weights.items():
            ...
            if weight > 0:  # Maximization
                ...
                normalized = 1 - (obj_values - obj_values.min()) / (
                    obj_values.max() - obj_values.min()
                )
    ...
    def create_optimization_visualizations(self):
        ...
        if self.pareto_points is not None:
            ...
            ax.scatter(
                self.pareto_points["T_sig_hybrid"],
                self.pareto_points["t5_hybrid"],
                c="red",
                s=100,
                alpha=0.8,
        ...
        pivot_table = self.df.pivot_table(
            values="composite_score", index="coupling_strength", columns="D"
        )
        ...
        ax = axes[1, 0]
        temp_norm = (self.df["T_sig_hybrid"] - self.df["T_sig_hybrid"].min()) / (
            self.df["T_sig_hybrid"].max() - self.df["T_sig_hybrid"].min()
        )
        time_norm = 1 - (self.df["t5_hybrid"] - self.df["t5_hybrid"].min()) / (
            self.df["t5_hybrid"].max() - self.df["t5_hybrid"].min()
        )
        ...
        im = ax.imshow(
            sensitivity_data.values.astype(float), cmap="RdBu_r", aspect="auto", vmin=-1, vmax=1
        )
    ...
    def generate_optimization_report(self):
        ...
        print(
            f"  - coupling_strength range: {high_temp_configs['coupling_strength'].min():.3f} - {high_temp_configs['coupling_strength'].max():.3f}"
        )
        print(
            f"  - D range: {high_temp_configs['D'].min():.2e} - {high_temp_configs['D'].max():.2e}"
        )
        print(
            f"  - Expected temperature: {high_temp_configs['T_sig_hybrid'].mean():.6e} ± {high_temp_configs['T_sig_hybrid'].std():.6e} K"
        )

        print(f"\nFor fastest detection:")
        print(
            f"  - coupling_strength range: {fast_detection_configs['coupling_strength'].min():.3f} - {fast_detection_configs['coupling_strength'].max():.3f}"
        )
        print(
            f"  - D range: {fast_detection_configs['D'].min():.2e} - {fast_detection_configs['D'].max():.2e}"
        )
        print(
            f"  - Expected detection time: {fast_detection_configs['t5_hybrid'].mean():.2e} ± {fast_detection_configs['t5_hybrid'].std():.2e} s"
        )

        print(f"\nMODEL SELECTION INSIGHTS:")
        print(f"  - Hybrid model consistently outperforms fluid model")
        print(
            f"  - Average temperature enhancement: {self.df['T_sig_hybrid'].mean() / self.df['T_sig_fluid'].mean():.1f}x"
        )
        print(
            f"  - Average detection speed improvement: {self.df['t5_fluid'].mean() / self.df['t5_hybrid'].mean():.1f}x"
        )
        ...
        return {
            ...
            "high_temp_params": {
                "coupling_range": (
                    high_temp_configs["coupling_strength"].min(),
                    high_temp_configs["coupling_strength"].max(),
                ),
                "D_range": (high_temp_configs["D"].min(), high_temp_configs["D"].max()),
            },
            "fast_detection_params": {
                "coupling_range": (
                    fast_detection_configs["coupling_strength"].min(),
                    fast_detection_configs["coupling_strength"].max(),
                ),
