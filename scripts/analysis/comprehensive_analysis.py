...
class HawkingRadiationAnalyzer:
    ...
    def load_data(self):
        ...
        print(
            f"  - Range of coupling_strength: {self.df['coupling_strength'].min():.3f} - {self.df['coupling_strength'].max():.3f}"
        )
        print(f"  - Range of D: {self.df['D'].min():.2e} - {self.df['D'].max():.2e}")
        print(
            f"  - Range of ratio_fluid_over_hybrid: {self.df['ratio_fluid_over_hybrid'].min():.3f} - {self.df['ratio_fluid_over_hybrid'].max():.3f}"
        )
    ...
    def test_specific_claims(self):
        ...
        print(
            f"95% CI for temperature ratio: [{temp_ratio_ci_lower:.3f}, {temp_ratio_ci_upper:.3f}]"
        )
        print(f"Mean absolute temperature increase: {temp_results['mean_difference']:.2e} K")
        print(
            f"95% CI for temperature difference: [{temp_results['ci_lower']:.2e}, {temp_results['ci_upper']:.2e}] K"
        )

        print(f"\nStatistical Test Results:")
        print(
            f"  • Paired t-test: t({temp_results['n_samples']-1}) = {temp_results['t_statistic']:.3f}, p = {temp_results['t_pvalue']:.2e}"
        )
        print(
            f"  • Wilcoxon signed-rank: W = {temp_results['wilcoxon_statistic']:.1f}, p = {temp_results['wilcoxon_pvalue']:.2e}"
        )
        print(
            f"  • Cohen's d = {temp_results['cohens_d']:.3f} ({temp_results['effect_size_interpretation']} effect)"
        )
        ...
        print(
            f"95% CI for speedup ratio: [{time_speedup_ci_lower:.3f}, {time_speedup_ci_upper:.3f}]"
        )
        print(f"Mean absolute time reduction: {time_results['mean_difference']:.2e} s")
        print(
            f"95% CI for time difference: [{time_results['ci_lower']:.2e}, {time_results['ci_upper']:.2e}] s"
        )

        print(f"\nStatistical Test Results:")
        print(
            f"  • Paired t-test: t({time_results['n_samples']-1}) = {time_results['t_statistic']:.3f}, p = {time_results['t_pvalue']:.2e}"
        )
        print(
            f"  • Wilcoxon signed-rank: W = {time_results['wilcoxon_statistic']:.1f}, p = {time_results['wilcoxon_pvalue']:.2e}"
        )
        print(
            f"  • Cohen's d = {time_results['cohens_d']:.3f} ({time_results['effect_size_interpretation']} effect)"
        )
    ...
    def generate_statistical_summary_report(self):
        ...
        if not self.significance_results:
            print(
                "⚠️  No statistical significance results available. Run test_specific_claims() first."
            )
            return
        ...
        if "temperature" in self.significance_results:
            ...
            print(
                f"• 95% Confidence Interval: [{temp['ratio_ci_lower']:.3f}, {temp['ratio_ci_upper']:.3f}]×"
            )
            print(
                f"• Statistical significance: p = {temp['t_pvalue']:.2e} {'✅ Significant' if temp['is_significant'] else '❌ Not significant'}"
            )
            print(
                f"• Effect size: Cohen's d = {temp['cohens_d']:.3f} ({temp['effect_size_interpretation']})"
            )
            print(f"• Statistical power: {temp['power']:.3f}")
            print(
                f"• 4× improvement claim: {'✅ SUPPORTED' if temp['claim_4x_supported'] else '❌ NOT SUPPORTED'}"
            )
        ...
        if "detection_time" in self.significance_results:
            ...
            print(
                f"• 95% Confidence Interval: [{time['speedup_ci_lower']:.3f}, {time['speedup_ci_upper']:.3f}]×"
            )
            print(
                f"• Statistical significance: p = {time['t_pvalue']:.2e} {'✅ Significant' if time['is_significant'] else '❌ Not significant'}"
            )
            print(
                f"• Effect size: Cohen's d = {time['cohens_d']:.3f} ({time['effect_size_interpretation']})"
            )
            print(f"• Statistical power: {time['power']:.3f}")
            print(
                f"• 16× speedup claim: {'✅ SUPPORTED' if time['claim_16x_supported'] else '❌ NOT SUPPORTED'}"
            )
        ...
        temp_claim = self.significance_results.get("temperature", {}).get(
            "claim_4x_supported", False
        )
        time_claim = self.significance_results.get("detection_time", {}).get(
            "claim_16x_supported", False
        )

        if temp_sig and time_sig:
            print(
                "✅ Both temperature and detection time improvements are statistically significant"
            )
        else:
    ...
    def statistical_characterization(self):
        ...
        if self.generate_plots:
            plt.tight_layout()
            plt.savefig(
                "results/analysis/parameter_distributions.png", dpi=300, bbox_inches="tight"
            )
            plt.show()
        else:
            print(
                "  📊 Plot generation disabled - use generate_plots=True to enable visualizations"
            )
        ...
        print(
            f"  - Parameter grid structure: {len(self.df['coupling_strength'].unique())} x {len(self.df['D'].unique())} = {len(self.df['coupling_strength'].unique()) * len(self.df['D'].unique())} total combinations"
        )
    ...
    def correlation_analysis(self):
        ...
        if "w_effective" in self.df.columns and "coupling_strength" in self.df.columns:
            ...
            if abs(corr_w_coupling) > 0.999:
                artifacts_excluded.append(
                    "w_effective (deterministic function of coupling_strength, r = {:.6f})".format(
                        corr_w_coupling
                    )
                )
                print(
                    "  ❌ w_effective: Excluded - deterministic function w_effective = 0.8027 × coupling_strength"
                )
        ...
        if "ratio_fluid_over_hybrid" in self.df.columns and "t5_hybrid" in self.df.columns:
            ...
            if abs(corr_ratio_hybrid) > 0.999:
                artifacts_excluded.append(
                    "ratio_fluid_over_hybrid (shares denominator with t5_hybrid, r = {:.6f})".format(
                        corr_ratio_hybrid
                    )
                )
                print(
                    "  ❌ ratio_fluid_over_hybrid: Excluded - mathematical dependency with t5_hybrid (ratio = t5_fluid/t5_hybrid)"
                )
        ...
        for col in numeric_df.columns:
            ...
            # Check for near-zero variance (effectively constant)
                ...
                constant_vars.append(col)
                artifacts_excluded.append(
                    f"{col} (near-constant, variance = {numeric_df[col].var():.2e})"
                )
        ...
        if "kappa_mirror" in self.df.columns and "D" in self.df.columns:
            ...
            if abs(corr_kappa_D) > 0.8:  # Strong deterministic relationship
                print(
                    f"  ⚠️  kappa_mirror: Strong deterministic relationship with D (r = {corr_kappa_D:.3f})"
                )
        ...
        print(
            "These exclusions ensure that all reported correlations reflect genuine physical relationships,"
        )
        ...
        for i in range(len(corr_matrix.columns)):
            for j in range(i + 1, len(corr_matrix.columns)):
                ...
                if abs(corr_val) > 0.3:
                    ...
                    print(
                        f"  {corr_matrix.columns[i]} ↔ {corr_matrix.columns[j]}: r = {corr_val:.3f} ({strength})"
                    )

        if not strong_correlations:
            print("  No significant correlations found in cleaned dataset")
            print(
                "  This suggests physical parameters are genuinely independent in the current parameter space"
            )
        ...
        plt.title(
            "Correlation Matrix: Physically Meaningful Relationships Only\n(Mathematical Artifacts Removed)",
            fontsize=14,
            fontweight="bold",
        )
        if self.generate_plots:
            plt.tight_layout()
            plt.savefig(
                "results/analysis/cleaned_correlation_heatmap.png", dpi=300, bbox_inches="tight"
            )
            plt.show()
        else:
        ...
        if len(cleaned_df.columns) >= 2:
            ...
            for i in range(len(cleaned_df.columns)):
                for j in range(i + 1, len(cleaned_df.columns)):
                    ...
                    if (var1 == "kappa_mirror" and var2 == "D") or (
                        var1 == "D" and var2 == "kappa_mirror"
                    ):
                        continue
            ...
            if meaningful_pairs:
                ...
                if self.generate_plots:
                    plt.tight_layout()
                    plt.savefig(
                        "results/analysis/physically_meaningful_relationships.png",
                        dpi=300,
                        bbox_inches="tight",
                    )
                    plt.show()
    ...
    def model_comparison_analysis(self):
        ...
        print(
            f"  - 95% CI for difference: [{temp_stats.get('ci_lower', 0):.6e}, {temp_stats.get('ci_upper', 0):.6e}] K"
        )
        print(f"  - Mean ratio (Hybrid/Fluid): {temp_ratio.mean():.6f}")
        print(
            f"  - 95% CI for ratio: [{temp_stats.get('ratio_ci_lower', 0):.6f}, {temp_stats.get('ratio_ci_upper', 0):.6f}]"
        )
        print(
            f"  - Statistical significance: p = {temp_stats.get('t_pvalue', 1):.2e} {'✅' if temp_stats.get('is_significant', False) else '❌'}"
        )
        print(
            f"  - Effect size: Cohen's d = {temp_stats.get('cohens_d', 0):.3f} ({temp_stats.get('effect_size_interpretation', 'Unknown')})"
        )
        ...
        print(
            f"  - 95% CI for difference: [{time_stats.get('ci_lower', 0):.2e}, {time_stats.get('ci_upper', 0):.2e}] s"
        )
        print(f"  - Mean speedup (Fluid/Hybrid): {speedup_ratio.mean():.3f}× faster")
        print(
            f"  - 95% CI for speedup: [{time_stats.get('speedup_ci_lower', 0):.3f}, {time_stats.get('speedup_ci_upper', 0):.3f}]×"
        )
        print(
            f"  - Statistical significance: p = {time_stats.get('t_pvalue', 1):.2e} {'✅' if time_stats.get('is_significant', False) else '❌'}"
        )
        print(
            f"  - Effect size: Cohen's d = {time_stats.get('cohens_d', 0):.3f} ({time_stats.get('effect_size_interpretation', 'Unknown')})"
        )
        ...
        print(
            f"  - Hybrid yields higher T_sig: {hybrid_better_temp}/{len(self.df)} ({100*hybrid_better_temp/len(self.df):.1f}%)"
        )
        print(
            f"    Statistical test: {temp_stats.get('t_pvalue', 1):.2e} ({'significant' if temp_stats.get('is_significant', False) else 'not significant'})"
        )
        print(
            f"  - Hybrid yields faster detection: {hybrid_better_time}/{len(self.df)} ({100*hybrid_better_time/len(self.df):.1f}%)"
        )
        print(
            f"    Statistical test: {time_stats.get('t_pvalue', 1):.2e} ({'significant' if time_stats.get('is_significant', False) else 'not significant'})"
        )
        ...
        if temp_stats.get("is_significant", False):
            ...
            print(
                f"     Magnitude: {temp_ratio.mean():.3f}× (95% CI: {temp_stats.get('ratio_ci_lower', 0):.3f}-{temp_stats.get('ratio_ci_upper', 0):.3f})"
            )
        else:
        ...
        if time_stats.get("is_significant", False):
            ...
            print(
                f"     Magnitude: {speedup_ratio.mean():.3f}× faster (95% CI: {time_stats.get('speedup_ci_lower', 0):.3f}-{time_stats.get('speedup_ci_upper', 0):.3f})"
            )
        else:
        ...
        if not temp_stats.get("claim_4x_supported", False):
            ...
            print(
                f"     Actual improvement: {temp_ratio.mean():.3f}× (95% CI: {temp_stats.get('ratio_ci_lower', 0):.3f}-{temp_stats.get('ratio_ci_upper', 0):.3f})"
            )

        if not time_stats.get("claim_16x_supported", False):
            print(f"  ❌ '16× faster detection' claim NOT SUPPORTED")
            print(
                f"     Actual speedup: {speedup_ratio.mean():.3f}× (95% CI: {time_stats.get('speedup_ci_lower', 0):.3f}-{time_stats.get('speedup_ci_upper', 0):.3f})"
            )
        ...
        ax = axes[0, 1]
        scatter = ax.scatter(
            self.df["coupling_strength"], time_ratio, c=self.df["D"], cmap="plasma", s=80, alpha=0.7
        )
        ax.axhline(y=1, color="r", linestyle="--", label="Equal performance")
        ax.set_xlabel("coupling_strength")
        ...
        performance_data = [
            hybrid_better_temp,
            len(self.df) - hybrid_better_temp,
            hybrid_better_time,
            len(self.df) - hybrid_better_time,
        ]
        labels = [
            "Higher T_sig\n(Hybrid)",
            "Higher T_sig\n(Fluid)",
        ...
        ax = axes[1, 1]
        scatter = ax.scatter(
            self.df["ratio_fluid_over_hybrid"],
            temp_ratio,
            c=self.df["coupling_strength"],
            cmap="coolwarm",
            s=80,
            alpha=0.7,
        )
    ...
    def scaling_analysis(self):
        ...
        ax = axes[1, 1]
        scatter = ax.scatter(
            self.df["kappa_mirror"],
            self.df["ratio_fluid_over_hybrid"],
            c=self.df["w_effective"],
            cmap="plasma",
            s=80,
            alpha=0.7,
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Comprehensive Analysis of Analog Hawking Radiation Dataset"
    )
    parser.add_argument(
        "--plots", action="store_true", help="Generate plots (can be slow on some computers)"
    )
