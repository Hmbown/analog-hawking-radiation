...
def load_datasets():
    ...
    try:
        ...
        print(f"Enhanced dataset: {len(enhanced_df)} total, {len(valid_enhanced_df)} valid configurations")
    except FileNotFoundError:
...
def compare_sample_sizes(original_df, enhanced_df):
    ...
    for effect_size in effect_sizes:
        ...
        print(f"    Original: {power_original:.3f} ({'Adequate' if power_original >= 0.8 else 'Insufficient'})")
        print(f"    Enhanced: {power_enhanced:.3f} ({'Adequate' if power_enhanced >= 0.8 else 'Insufficient'})")
...
def compare_parameter_coverage(original_df, enhanced_df):
    ...
    for param in common_params:
        if param in original_df.columns and param in enhanced_df.columns:
            ...
            print(f"\n{param}:")
            print(f"  Original:")
            print(f"    Range: {orig_values.min():.3e} - {orig_values.max():.3e} (factor: {orig_range:.1f}×)")
            print(f"    Uniformity (CV): {orig_cv:.3f}")
            print(f"  Enhanced:")
            print(f"    Range: {enh_values.min():.3e} - {enh_values.max():.3e} (factor: {enh_range:.1f}×)")
...
def compare_physical_regimes(original_df, enhanced_df):
    ...
    if 'normalized_density' in enhanced_df.columns:
        ...
        critical = ((enhanced_df['normalized_density'] >= 1e-3) &
                   (enhanced_df['normalized_density'] <= 1e3)).sum()
...
def compare_statistical_robustness(original_df, enhanced_df):
    ...
    if 'coupling_strength' in original_df.columns and 'coupling_strength' in enhanced_df:
        ...
        print(f"\nCoupling Strength:")
        print(f"  Original: mean = {orig_cs.mean():.3f}, CI = [{orig_ci_low:.3f}, {orig_ci_high:.3f}]")
...
def generate_summary_comparison(original_df, enhanced_df):
    ...
    print(f"  Sample size: {len(original_df)} → {len(enhanced_df)} ({size_improvement:.1f}× increase)")

    # Parameter dimensions
    param_improvement = len(enhanced_df.columns) / len(original_df.columns)
    print(f"  Parameters: {len(original_df.columns)} → {len(enhanced_df.columns)} ({param_improvement:.1f}× increase)")
    ...
    power_orig_medium = 1 - stats.t.cdf(stats.t.ppf(1 - 0.025, n_orig-2), n_orig-2, 0.5 * np.sqrt(n_orig/2))
    power_enh_medium = 1 - stats.t.cdf(stats.t.ppf(1 - 0.025, n_enh-2), n_enh-2, 0.5 * np.sqrt(n_enh/2))
    power_improvement = power_enh_medium / power_orig_medium if power_orig_medium > 0 else float('inf')

    print(f"  Statistical power (medium effects): {power_orig_medium:.3f} → {power_enh_medium:.3f} ({power_improvement:.1f}×)")
