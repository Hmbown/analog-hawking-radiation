...
class HawkingOptimization:
    ...
    def plot_convergence(self, results: Dict[str, Any]):
        ...
        try:
            ...
            plt.savefig(output_dir / f"optimization_convergence_{'hybrid' if self.use_hybrid else 'standard'}.png", 
        ...
        if results['best_parameters']:
            ...
            try:
                ...
                if final_result.kappa and final_result.T_H_K is not None:
                    sig_temp_str = f"{final_result.T_sig_K:.2e} K" if final_result.T_sig_K is not None else "N/A"
                    snr_str = (
                        f"{final_result.T_sig_K/30:.2f} (T_sys=30K)"
                        if final_result.T_sig_K is not None else "N/A"
                    )
                    report += (
                        "\nDerived Quantities:\n"
...
def main():
    parser = argparse.ArgumentParser(description="Bayesian optimization for Hawking radiation detection")
    parser.add_argument("--hybrid", action="store_true", help="Use hybrid plasma mirror model")
    parser.add_argument("--n-calls", type=int, default=100, help="Number of optimization iterations")
