...
def run_single_configuration(
    ...
    try:
        model = MaxwellFluidModel(
            plasma_density=n_e, laser_wavelength=lambda_l, laser_intensity=I_0
        )
...
def run_gradient_catastrophe_sweep(
    n_samples: int = 500,
...
def analyze_catastrophe_boundaries(results: List[Dict]) -> Dict:
    ...
    for mode in [
        ...
        breakdown_stats[mode] = {"count": count, "rate": count / len(results) if results else 0}
    ...
    return {
        ...
        "parameter_boundaries": {
            ...
            "max_valid_gradient": (
                max([r["gradient_factor"] for r in valid_results]) if valid_results else 0
            ),
        },
    }
...
def generate_catastrophe_plots(sweep_data: Dict, output_dir: str = "results/gradient_limits"):
    ...
    if valid_results:
        ax1.scatter(
            [r["a0"] for r in valid_results],
            [r["kappa"] for r in valid_results],
            c="green",
            alpha=0.6,
            label="Valid Physics",
            s=30,
    ...
    if invalid_results:
        ax1.scatter(
            [r["a0"] for r in invalid_results],
            [max(r["kappa"], 1e6) for r in invalid_results],
            c="red",
            alpha=0.6,
    ...
    if valid_results:
        ...
        scatter = ax3.scatter(
            ...
            alpha=0.7,
            cmap="viridis",
            norm=plt.Normalize(vmin=kappa_vals.min(), vmax=kappa_vals.max()),
        )
...
def main():
    ...
    parser.add_argument(
        "--n-samples", type=int, default=500, help="Number of parameter combinations to test"
    )
    parser.add_argument(
        "--output", type=str, default="results/gradient_limits", help="Output directory for results"
    )
    parser.add_argument(
        "--thresholds",
        type=str,
    ...
    breakdown_stats = {
        name: stats
        for name, stats in analysis["breakdown_statistics"].items()
