...
class StatisticalAnalyzer:
    ...
    def calculate_detection_probability(
        self, results: List[Dict[str, Any]], observation_time: float = 3600.0
    ) -> Dict[str, Any]:
    ...
    def analyze_scaling_requirements(self, results: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        if len(log_params) > 2:
            ...
            scaling_analysis = {"error": "Insufficient data for power law fitting"}
            ...
            if current_median > 0:
                ...
                if "power_law_exponent" in scaling_analysis:
                    ...
                    scaling_requirements[f"target_{target}s"] = {
                        ...
                        "feasibility": (
                            "achievable" if required_intensity_increase < 1e4 else "challenging"
                        ),
                    }
        ...
        return {
            ...
            "parameter_ranges": {
                ...
                "detection_time_range": [
                    float(np.min(detection_times)),
                    float(np.max(detection_times)),
                ],
            },
        }
    ...
    def generate_report(self, results: List[Dict[str, Any]]) -> str:
        ...
        # Basic statistics
        detection_times = [
            r.get("t5sigma_s", 1e6) for r in enhanced_results if r.get("t5sigma_s") is not None
        ]
        snr_values = [
            r.get("signal_to_noise_ratio", 0.0)
            for r in enhanced_results
            if r.get("signal_to_noise_ratio") > 0
        ]
...
def main():
    parser = argparse.ArgumentParser(
        description="Statistical analysis for Hawking radiation detection"
    )
    ...
    parser.add_argument(
        "--observation-time", type=float, default=3600, help="Observation time in seconds"
    )
    ...
    detection_stats = analyzer.calculate_detection_probability(
        enhanced_results, args.observation_time
    )
