...
def main() -> int:
    ...
    p.add_argument(
        "--real-warpx",
        action="store_true",
        help="Run real WarpX simulation (falls back to synthetic if not available)",
    )
    ...
    p.add_argument(
        "--nd-npz", type=str, default=None, help="Path to nD grid NPZ (x[,y[,z]], vx,vy[,vz], c_s)"
    )
    p.add_argument(
        "--nd-scan-axis", type=int, default=0, help="Axis along which to scan for horizon crossings"
    )
    p.add_argument(
        "--nd-patches", type=int, default=64, help="Max horizon patches to aggregate for graybody"
    )
    ...
    if args.nd_npz or args.nd_h5_in:
        ...
        if agg.get("success"):
            ...
            t = (
                float(sweep_time_for_5sigma(np.array([args.Tsys]), np.array([B]), Ts)[0, 0])
                if Ts > 0
                else float("inf")
            )
            summary.update(
...
""".strip()
            ...
            result = subprocess.run(
                [args.warpx_exec, "warpx.in"], capture_output=True, text=True, cwd="."
            )
            if result.returncode != 0:
                raise subprocess.CalledProcessError(
                    result.returncode, [args.warpx_exec, "warpx.in"], result.stdout, result.stderr
                )
            ...
            # Generate synthetic
            subprocess.run(
                [sys.executable, "scripts/generate_synthetic_openpmd_slice.py"], check=True, cwd="."
            )
    ...
    if kappa:
        spec = calculate_hawking_spectrum(
            float(kappa[0]),
            graybody_profile={"x": x, "v": v, "c_s": cs},
            graybody_method=args.graybody,
