...
class PerformanceMonitor:
    ...
    def __init__(self, experiment_id: str, update_interval: float = 30.0):
        ...
        self.logger.info(
            f"Initialized comprehensive performance monitor for experiment {experiment_id}"
        )
    ...
    def _collect_experiment_metrics(self) -> ExperimentMetrics:
        ...
        # Check for performance issues
        performance_issues = self._detect_performance_issues(success_rate, average_simulation_time)
    ...
    def _detect_performance_issues(
        self, success_rate: float, avg_simulation_time: float
    ) -> List[str]:
    ...
    def _check_performance_issues(
        self, system_metrics: SystemMetrics, experiment_metrics: ExperimentMetrics
    ) -> None:
    ...
    def _collect_optimization_metrics(self) -> OptimizationMetrics:
        ...
        if len(self.improvement_history) >= 2:
            improvement_rate = (
                np.mean(self.improvement_history[-10:]) if self.improvement_history else 0.0
            )
        else:
        ...
        if len(self.parameter_convergence_history) >= 2:
            parameter_convergence = (
                np.mean(self.parameter_convergence_history[-10:])
                if self.parameter_convergence_history
                else 0.0
            )
        else:
    ...
    def _check_optimization_performance(self, optimization_metrics: OptimizationMetrics) -> None:
        ...
        if (
            optimization_metrics.convergence_status == "stalled"
            and self.optimization_iterations > 10
        ):
            self._create_alert(
                severity="medium",
        ...
        if (
            optimization_metrics.optimization_efficiency < 0.001
            and self.optimization_iterations > 20
        ):
            self._create_alert(
                severity="low",
                category="optimization",
    ...
    def _analyze_experiment_performance(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "progress": {
                "phase_progress": self.phase_progress,
                "estimated_completion": self.experiment_metrics_history[
                    -1
                ].estimated_phase_completion,
    ...
    def _analyze_resource_utilization(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "efficiency_rating": (
                ...
                else "good" if overall_efficiency > 0.6 else "needs_improvement"
            ),
        }
    ...
    def _generate_performance_recommendations(
        ...
        if "error" not in system_summary:
            ...
            if memory_status == "high":
                recommendations.append(
                    "Optimize memory usage by reducing batch sizes or using more efficient data structures"
                )
            if disk_status == "high":
                recommendations.append(
                    "Clean up temporary files and consider archiving old results"
                )
        ...
        if "error" not in optimization_summary:
            ...
            if progress_assessment in ["stalled_needs_attention", "struggling_needs_intervention"]:
                recommendations.append(
                    "Adjust optimization strategy or explore different parameter regions"
                )
    ...
    def _identify_critical_issues(self) -> List[str]:
        ...
        recent_critical_alerts = [
            alert for alert in self.alerts[-20:] if alert.severity == "critical"
        ]
    ...
    def _calculate_health_score(
        ...
        if "error" not in experiment_summary:
            ...
            experiment_score += (
                0.5 if experiment_summary["success_rate"]["status"] == "good" else 0.2
            )
            experiment_score += (
                0.3 if experiment_summary["simulation_efficiency"]["status"] == "efficient" else 0.1
            )
            experiment_score += (
                0.2 if experiment_summary["progress"]["status"] == "on_track" else 0.1
            )
    ...
    def _generate_comprehensive_text_report(self, report: PerformanceReport) -> str:
        ...
        if "error" not in report.experiment_performance_summary:
            ...
            text += (
                f"Progress: {exp['progress']['phase_progress']:.1%} ({exp['progress']['status']})\n"
            )
        else:
    ...
    def update_simulation_stats(self, completed: int, successful: int, compute_time: float) -> None:
    ...
    def get_current_metrics(self) -> Tuple[SystemMetrics, ExperimentMetrics]:
        ...
        experiment_metrics = (
            self.experiment_metrics_history[-1] if self.experiment_metrics_history else None
        )
