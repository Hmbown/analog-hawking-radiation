...
class ResultAggregator:
    ...
    def _setup_logging(self) -> None:
        ...
        logging.basicConfig(
            ...
            handlers=[logging.FileHandler(log_dir / "aggregation.log"), logging.StreamHandler()],
        )
        if sns is None:
            logging.getLogger(__name__).info(
                "Seaborn not installed; visualizations will be skipped."
            )
        ...
        for i, phase1 in enumerate(phase_names):
            for j, phase2 in enumerate(phase_names[i + 1 :], i + 1):
                ...
                for metric in common_metrics:
                    if (
                        len(metrics1[metric]) > 5 and len(metrics2[metric]) > 5
    ...
    def _extract_phase_metrics(self, phase_name: str) -> Dict[str, List[float]]:
        ...
        metrics = {"detection_time": [], "kappa": [], "signal_temperature": []}
    ...
    def _plot_cross_phase_correlation(
        self, aggregate: ExperimentAggregate, output_dir: Path
    ) -> None:
        ...
        for (
            phase1,
            phase2,
        ), corr in aggregate.cross_phase_correlation.correlation_coefficients.items():
    ...
    def _plot_detection_time_distribution(self, output_dir: Path) -> None:
        ...
        df = pd.DataFrame({"detection_time": all_detection_times, "phase": phase_labels})
    ...
    def _plot_success_rates(self, aggregate: ExperimentAggregate, output_dir: Path) -> None:
        ...
        successful_simulations = [
            aggregate.phase_summary[p]["successful_simulations"] for p in phases
        ]
        ...
        ax1.set_xticks(range(len(phases)))
        ax1.set_xticklabels(
            [p.replace("phase_", "").replace("_", " ").title() for p in phases], rotation=45
        )
    ...
    def generate_comprehensive_report(self, aggregate: ExperimentAggregate) -> None:
        ...
        with open(report_file, "w") as f:
            ...
            f.write(
                f"Best Detection Time: {aggregate.best_detection_time:.2e} s\n"
                if aggregate.best_detection_time is not None
                else "Best Detection Time: N/A\n"
            )
            f.write(
                f"Best Surface Gravity: {aggregate.best_kappa:.2e} s⁻¹\n"
                if aggregate.best_kappa is not None
                else "Best Surface Gravity: N/A\n"
            )
            f.write(
                f"Best Signal-to-Noise Ratio: {aggregate.best_snr:.2f}\n\n"
                if aggregate.best_snr is not None
                else "Best Signal-to-Noise Ratio: N/A\n\n"
            )
            ...
            for phase_name, summary in aggregate.phase_summary.items():
                ...
                f.write(
                    f"  Best Detection Time: {summary['best_detection_time']:.2e} s\n"
                    if summary["best_detection_time"] is not None
                    else "  Best Detection Time: N/A\n"
                )
                f.write(
                    f"  Best Surface Gravity: {summary['best_kappa']:.2e} s⁻¹\n"
                    if summary["best_kappa"] is not None
                    else "  Best Surface Gravity: N/A\n"
                )
            ...
            for (
                ...
                corr,
                p_val,
            ) in aggregate.cross_phase_correlation.significant_correlations:
            ...
            if "detection_probability_1h" in stats:
                ...
                f.write(
                    f"  3σ detection probability: {prob_1h['detection_probability_3sigma']:.1%}\n"
                )
                f.write(
                    f"  5σ detection probability: {prob_1h['detection_probability_5sigma']:.1%}\n"
                )
                f.write(
                    f"  6σ detection probability: {prob_1h['detection_probability_6sigma']:.1%}\n"
                )
