...
def create_synthetic_dataset() -> pd.DataFrame:
    ...
    for i in range(n_samples):
        ...
        intensity_W_m2 = (
            (a0 * 9.11e-31 * 3e8 * 2 * np.pi * 3e8 / (800e-9 * 1.6e-19)) ** 2 * 3e8 * 8.85e-12 / 2
        )
...
def analyze_configuration_detection_feasibility(
    row: pd.Series, analyzer: DetectionFeasibilityAnalyzer
) -> Dict[str, Any]:
    ...
    try:
        ...
        try:
            ...
            frequencies = np.logspace(
                np.log10(row["peak_frequency_Hz"] / 10),
                np.log10(row["peak_frequency_Hz"] * 10),
                100,
            )
        ...
        # Plasma parameters
        plasma_params = {"temperature": 1e6, "density": row["plasma_density_m3"]}  # 1 MK typical
...
def generate_detection_feasibility_plots(results_df: pd.DataFrame, output_dir: str):
    ...
    if len(valid_data) > 0:
        if "a0" in valid_data.columns:
            ...
            axes[0, 0].scatter(
                valid_data["coupling_strength"], valid_data["best_snr"], alpha=0.6, s=50
            )
    ...
    if len(valid_data) > 0:
        axes[1, 0].scatter(
            valid_data["hawking_temperature_K"], valid_data["best_snr"], alpha=0.6, s=50
        )
        ...
        axes[1, 0].axhline(
            y=5, color="red", linestyle="--", alpha=0.7, label="5σ detection threshold"
        )
        axes[1, 0].legend()
    ...
    if len(valid_data) > 0:
        ...
        method_performance = (
            valid_data.groupby("best_method")
            .agg(
                {
    ...
    if len(valid_data) > 0:
        ...
        feasibility_map = valid_data.pivot_table(
            values="best_snr", index="a0", columns="plasma_density_m3", aggfunc="mean"
        )

        # SNR heatmap
        im1 = axes[0].imshow(
            np.log10(feasibility_map.values), aspect="auto", origin="lower", cmap="viridis"
        )
        ...
        feasibility_numeric_map = valid_data.pivot_table(
            values=feasibility_numeric, index="a0", columns="plasma_density_m3", aggfunc="mean"
        )

        im2 = axes[1].imshow(
            feasibility_numeric_map.values, aspect="auto", origin="lower", cmap="RdYlGn"
        )
...
def generate_summary_statistics(results_df: pd.DataFrame) -> Dict[str, Any]:
    ...
    challenging_configs = len(
        results_df[(results_df["best_snr"] >= 1) & (results_df["best_snr"] < 5)]
    )
    ...
    summary = {
        ...
        "feasibility_percentage": (
            100 * feasible_configs / total_configs if total_configs > 0 else 0
        ),
    ...
    if "hawking_temperature_K" in results_df.columns:
        summary["parameter_ranges"]["hawking_temperature"] = {
            ...
            "optimal_range": [
                results_df.loc[results_df["best_snr"].idxmax(), "hawking_temperature_K"]
            ],
        }

    # Detection method statistics
    method_stats = (
        results_df.groupby("best_method")
        .agg({"best_snr": ["count", "mean", "max"], "detection_probability": "mean"})
        .round(3)
    )
...
def generate_eli_facility_recommendations(results_df: pd.DataFrame) -> Dict[str, Any]:
    ...
    recommendations = {
        "facility_requirements": {
            "laser_systems": {
                "intensity_range": [
                    results_df["laser_intensity_W_m2"].min(),
                    results_df["laser_intensity_W_m2"].max(),
                ],
            ...
            "target_systems": {
                "plasma_density_range": [
                    results_df["plasma_density_m3"].min(),
                    results_df["plasma_density_m3"].max(),
                ],
...
def main():
    ...
    parser.add_argument(
        "--output", type=str, default="results/detection_feasibility", help="Output directory"
    )
    parser.add_argument(
        "--n-configs", type=int, default=50, help="Number of configurations to analyze"
    )
    parser.add_argument(
    ...
    if args.generate_report:
        ...
        for _, row in top_results.iterrows():
            try:
                signal = SignalCharacteristics(
                    ...
                    power_density=row.get("signal_power_W", 1e-25)
                    / (row["peak_frequency_Hz"] / 10),
                    signal_temperature=row.get(
                        "signal_temperature_K", row["hawking_temperature_K"] * 0.1
                    ),
                    pulse_duration=1e-12,
                    rise_time=1e-13,
                    repetition_rate=1.0,
                ...
                plasma_params = {"temperature": 1e6, "density": row["plasma_density_m3"]}
        ...
        if detailed_assessments:
            report = analyzer.generate_feasibility_report(
                detailed_assessments, output_dir / "comprehensive_detection_feasibility_report.md"
            )
    ...
    if len(results_df) > 0:
        ...
        print(
            f"Feasibility rate: {100*feasible_count/total_count:.1f}%" if total_count > 0 else "0%"
        )
